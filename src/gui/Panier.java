...
public class Panier {
    ...
    public Panier()  {
         ...
         List<ArrayList> listeEventa = Gui.Login.panier;
    ...
    image.setImage(img);
         ...
         f.getToolbar().addCommandToSideMenu("mes commandes",null, new ActionListener() {
             @Override
             public void actionPerformed(ActionEvent d) {
                 gui.Mescommandes commpage = new gui.Mescommandes();
                 commpage.getF().show();
             }
         });
         ...
         f.getToolbar().addCommandToSideMenu("passer commande",null, new ActionListener() {
             ...
             public void actionPerformed(ActionEvent d) {
                 ...
                 comm.setEtat(0);
                 comm.setId_client(Gui.Login.loggduser.getId());
                 comm.setPrix_tot(prix_tot);
                 ServiceCommande serco= new ServiceCommande();
                 serco.Create(comm);
                 int idf=0;
                 idf=serco.getlid(Gui.Login.loggduser.getId());
//ligne
                  for(Produit e : listeP)
                    {
                        Lignecommande lcomm = new Lignecommande();
                        lcomm.setQte(1);
                        lcomm.setId_client(Gui.Login.loggduser.getId());
